...
| 必要测试 | PNG/JPEG/GIF 尺寸测试；PNG CRC 标准向量、IHDR 字段、header-only 截断、块长度越界、缺少 IEND、尾随数据和 IDAT CRC 损坏测试；CLI smoke 覆盖递归、路径越界、通过/失败报告、大小写不一致及各类无效输入 | 已覆盖当前功能范围；继续按新增行为补测 |
