...
| 必要测试 | PNG/JPEG/GIF 尺寸头单元测试及 CLI 端到端 smoke；覆盖目录递归、路径越界拒绝、通过、失败报告、大小写不一致、无效/缺失/类型错误输入与输出路径冲突 | 已满足当前功能范围；继续按新增行为补测 |
| 跨平台 CI | [GitHub Actions 运行 #35212045289](https://github.com/hxiuzheng/media-pack-audit/actions/runs/35212045289)：Ubuntu、Windows 两个 job 均成功 | 已满足 |
